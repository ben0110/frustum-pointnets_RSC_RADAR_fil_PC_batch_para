...
TEST_DATASET = provider.RadarDataset_bbox_CLS('pc_radar_2','KITTI',npoints=NUM_POINT, split='val',rotate_to_center=False, one_hot=True,all_batches = True, translate_radar_center=False, store_data=True, proposals_3 =True ,no_color=True)
...
def train():
    ...
    with tf.Graph().as_default():
        ...
        for epoch in range(MAX_EPOCH):
            ...
            heading_cls_list, \
            center_list =  eval_one_epoch(sess, ops, test_writer)
...
def eval_per_frame(id_list,indice_box_list,GT_box_list,pred_box_list,IOU3d,score_list):
    ...
    corners_GT_frame,id_list_GT =provider.load_GT_eval(id_list_frame[len(id_list_frame)-1],'KITTI')
    print("****************************************************************")
...
def eval_one_epoch(sess, ops, test_writer):
    ...
    for batch_idx in range(num_batches):
        ...
        summary, step, _, loss_val, iou2ds, iou3ds, box_pred_nbr,centers_pred_val, heading_scores, heading_residuals, size_scores, size_residuals = \
            sess.run([ops['merged'], ops['step'], ops['train_op'], ops['loss'],
                      ops['end_points']['iou2ds'], ops['end_points']['iou3ds'], ops['end_points']['box_pred_nbr'],ops['end_points']['center'],ops['end_points']['heading_scores'], ops['end_points']['heading_residuals'], \
    ...
    return size_residual_GT, size_class_GT, heading_res_GT,\
                                                        heading_class_GT, center_GT, score_list,\
                                                        size_res_list, size_cls_list, heading_res_list,\
                                                        heading_cls_list,\
                                                        center_list
